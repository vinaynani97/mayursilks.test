...
function NotificationRow({
  ...
  const inner = (
    ...
    >
      {/* Icon */}
      <div className="w-8 h-8 rounded-full bg-white border border-gray-100 flex items-center justify-center flex-shrink-0 text-base shadow-sm">
        {n.icon ?? "🔔"}
      </div>
      ...
      <div className="flex-1 min-w-0">
        <p className={cn(
          "font-jost text-sm leading-snug text-gray-800",
...
export default function CustomerNotificationBell() {
  const {
    ...
    clearRead,
    loadMore,
  } = useNotifications({ pollInterval: 30_000, dropdownLimit: 8 });
  ...
  return (
    <div className="relative" ref={panelRef}>
      ...
      <button
        ...
        className="hidden sm:flex p-2 text-gray-600 hover:text-primary-500 transition-colors rounded-md hover:bg-primary-50 relative"
          ...
          >
            ...
            <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100">
              <div className="flex items-center gap-2">
                ...
                {unreadCount > 0 && (
                  <span className="text-[10px] bg-primary-100 text-primary-600 font-bold px-1.5 py-0.5 rounded-full">
                    {unreadCount} new
                  </span>
                )}
              </div>
              ...
              {notifications.length > 0 && (
                <div className="divide-y divide-gray-50">
                  {orderedGroups.map((group) => {
                    ...
                    return (
                      <div key={group}>
                        <p className="px-4 pt-3 pb-1 font-josefin text-[10px] font-bold text-gray-400 uppercase tracking-widest">
                          {group}
                        </p>
                        {items.map((n) => (
                          <NotificationRow
